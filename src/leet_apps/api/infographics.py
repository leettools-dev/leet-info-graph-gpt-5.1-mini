...
@router.post("/generate", response_model=Infographic)
async def generate(info: InfographicCreate):
    # Create deterministic title from prompt
    title = (info.prompt[:40] + "...") if len(info.prompt) > 40 else info.prompt
    svg = generate_svg(title=title, prompt=info.prompt, sources=info.sources)

    infographic_id = str(uuid.uuid4())
    created_at = datetime.utcnow()
    image_url = f"/api/infographics/{infographic_id}/image?format=svg"
    layout_meta = {"template": "simple_v1", "source_count": len(info.sources)}
    ...
    return Infographic(id=infographic_id, session_id=info.session_id, image_url=image_url, layout_meta=layout_meta, created_at=created_at)
